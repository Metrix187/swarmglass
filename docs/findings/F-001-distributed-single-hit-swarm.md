...
The `og_only` reaches are the most interesting behavioural detail: the client parses `<meta property="og:see_also">`, which ordinary link extractors ignore. That is a deliberate feature, not a generic crawler default. Whether it understands Open Graph or simply harvests any URL-shaped `content=` attribute is an open question with an experiment attached (below). On the second day the pool also reached the JSON-LD-only page, which rules out "Open Graph reader and nothing else": it takes URLs out of at least two kinds of head metadata that no declared crawler on the mirror has touched. The header-only and link-only pages remain unreached, so the width of the net is still open. SGX-011's first result points the same way as the JSON-LD reach: the URL inside an `og:image` property was fetched as a document, so the reader is a URL harvester over metadata rather than a consumer of what the metadata says. The bare-text carrier, followed an hour after it was shown, settled it further: the reader scans the raw source for URL-shaped strings and needs no markup at all. Which carriers it does not take, the HTML comment above all, is what the remaining arms measure.
