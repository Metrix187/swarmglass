...
- 21:31 UTC the same day: the carrier rides on every article the pool fetches (`host_page: "*"`), leaving out the target and the other experiments' targets so no other variable is touched. A member rendered Main_Page about once an hour, and with the pool sampling what it sees each arm needs many exposures, not one; on every article, every member request is one. `host_page_history` keeps the earlier host in force for events before the switch, so the comparison does not backdate exposures nobody had. Version unchanged, so the revision ids already in the pool's queue stay attributable.
